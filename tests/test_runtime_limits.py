"""Regressions for runtime limits, event-loop handling and fetch policy.

Each asserts the behaviour through the real entry point. The recurring failure
in earlier versions was testing an adjacent surface: configuration loading rather
...
# ---- MCP under a running event loop ------------------------------------ #
...
# ---- concurrency floor at the authoritative boundary ------------------- #
...
def test_placeholder_detection_targets_the_sentinel_not_the_owner():
    ...
    text = (_repo("attribution-suite") / "tests/test_release_and_mcp.py").read_text()
...
def test_mcp_does_not_present_a_raw_identifier_as_the_conclusion():
    """On a real run `_top_entity` returned
...
def test_fetcher_never_writes_a_cache_into_the_working_directory():
    """`cache_dir` defaulted to `Path(".eae-cache")`, so retrieved bytes
