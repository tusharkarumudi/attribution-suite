...
Each reads the artifact or executes the path a review examined, not an
adjacent surface. B1 exists because the earlier version fixed the *test* that
...
def test_top_level_manifest_tampering_is_detected(tmp_path, field, value):
    ...
    manifest's own assertions. a review rewrote case_ref, authorization,
...
# ---- blocked reconciliation without evidence ---------------------------- #
...
def test_ci_clones_siblings_before_installing(pkg):
    """`pip install -e ".[dev]"` ran before the sibling checkout. The
    dependent packages pin attribution-graph>=2.0.1, which is not on PyPI until
...
def test_tag_mismatch_explains_how_to_rehearse(pkg):
    """The documented rehearsal used `v2.0.0-rc1` against a `2.0.0`
...
def test_security_job_bootstraps_siblings_too(pkg):
    """The test job was fixed and the security job was not: it also runs
    `pip install -e .`, and the dependants pin attribution-graph>=2.0.1, which
...
def test_standalone_verifier_rejects_manifest_forgery(tmp_path):
    """The envelope check went into the installed verifier and not into the
...
def test_every_network_tool_refuses_without_authorization(tool, args):
    """The check lived inside `explain_ads_txt`, so `attribute_domain`
...
def test_index_crawl_rejects_non_positive_concurrency():
    """Engine validated it; `paytrace-index build --concurrency 0` is a
...
def test_production_publish_does_not_skip_existing(pkg):
    """`skip-existing: true` tolerates a filename collision silently, so a
