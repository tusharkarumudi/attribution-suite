...
def test_all_four_packages_report_one_version():
    ...
    v = set(versions().values())
    assert len(v) == 1, f"versions diverge: {versions()}"
...
def test_ads_txt_to_seller_to_name_to_expansion():
    ...
    body = json.dumps({"sellers": [
        {"seller_id": "pub-111", "name": "HOÀNG PHÚ LINH",
    ...
    class Idx:
        def sellers_for_name(self, n):
            return [("google.com", "pub-111", "")] if "linh" in n.lower() else []
...
def test_handle_module_absence_is_reported_not_crashed(monkeypatch):
    ...
    exp = asyncio.run(expand_from_name("Hoang Phu Linh"))
...
def test_seller_name_classification_is_shared_not_duplicated():
    ...
    assert classify_seller_name("HOÀNG PHÚ LINH") is SellerNameKind.NATURAL_PERSON
...
def test_pivot_loop_recovers_identity_from_a_sibling():
    ...
    sibling = ('<script src="https://www.googletagmanager.com/gtag/js?id='
               'G-SHARED123"></script><p>Operated by Dai Nguyen</p>')
...
def test_real_policy_engine_and_real_fetcher_are_compatible():
    """RB-01. PolicyEngine.evaluate is `async (fetcher, url)` returning a
...
def test_verify_command_does_not_execute_package_code():
    """RB-04. `attribution verify` ran the verify.py inside the evidence
...
def test_case_egress_survives_load_and_reaches_transport():
    """RA-02. `CaseScope` had no egress field, so an `egress:` block in a case
...
def test_provider_network_mismatch_is_rejected():
    """RA-04. `provider: oxylabs` with `network: datacenter` routed to the
    ...
    with _pytest.raises(ProxyError, match="serves"):
        ...
        # datacenter label on it is the exact mislabelling the audit found.
...
def test_evidence_records_every_retrieval(scenario):
    """RA-01. 404s returned before recording, redirect hops were unrecorded,
